...
func calculateArea(x: Double, y: Double) -> Double? {
    guard x > 0.0 else {
        print("x can't be a negative value.")
        return nil
    }
    
    
    guard y > 0.0 else {
        print("y can't be a negative value.")
        return nil
    }
...
print(calculateArea(x: 5, y: 3))

if let unwrappedCalculatedArea = calculateArea(x: 5, y: 3) {
    print(unwrappedCalculatedArea)
}

print(calculateArea(x: -3, y: 5))
/*:
...
if let unwrappedAdd = add(x: 5, y: 18) {
    print(unwrappedAdd)
}

print(add(x: nil, y: 15))
/*:
...
func createUser() -> User? {
    guard let unwrappedFirstNameTextField = firstNameTextField.text, let unwrappedLastNameTextField = lastNameTextField.text, let unwrappedAgeTextField = ageTextField.text else {
        return nil
    }
    
    return User(firstName: unwrappedFirstNameTextField, lastName: unwrappedLastNameTextField, age: unwrappedAgeTextField)
}
/*:
 ...
 */
if let myUser = createUser() {
    print("\(myUser.firstName) \(myUser.lastName). Age: \(myUser.age)")
}
